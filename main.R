...
# scaling functions
standardise_factory <- function(b = c(0, 1)) {
...
scale <- list()
scale[["penalty"]] <- standardise_factory(b = c(10^-1, 10^-4))
scale[["tree_depth"]] <- standardise_factory(b = c(1, 16))
scale[["cost_complexity"]] <- standardise_factory()
scale[["num_comp"]] <- standardise_factory(c(1, 22))
...
logit[["grid"]] <- tibble(
  penalty = runif(150) %>% scale$penalty(),
  num_comp = runif(150) %>% scale$num_comp()
  )
logit[["recipe"]] <- recipe(children ~ ., data = train) %>% 
  ...
  step_pca(-children, num_comp = tune())
